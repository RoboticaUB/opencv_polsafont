...
class GoPiGo3:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/cmd_detection", String, self.callbackDetection)
    ...
    def callbackDetection(self, detectionMessage):    
        if detectionMessage.data == "negative":
            rospy.logwarn("TARGET NOT FOUND")

        if detectionMessage.data == "positive" and self._state == "START":
    ...
    def callbackLaser(self, scan):
        if self._state == "START":
            ...
            # BASIC NAV PROGRAM
            if closestDistance < self._distanceLaser and -60 < angleClosestDistance < 60:
